    ...
    <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-bg-primary">
      ...
      <Container className="relative z-10 flex flex-col items-center text-center">
        ...
        <motion.h1
          ...
          className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-transparent bg-clip-text bg-linear-to-r from-text-primary via-accent-blue to-accent-purple mb-4 animate-fade-in tracking-tight"
        >
          {about?.name}
        </motion.h1>
        ...
        <motion.p
          ...
          className="font-medium text-lg sm:text-xl md:text-2xl text-text-secondary tracking-normal mb-5"
        >
          {about?.title}
        </motion.p>

        {/* Tagline / Tech Stack Pills */}
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.4, delay: 0.35 }}
          className="flex flex-wrap items-center justify-center gap-2 sm:gap-2.5 max-w-2xl mb-8"
        >
          {about?.tagline
            ?.split(/[•|]/)
            .map((item) => item.trim())
            ...
            .map((tech, idx) => (
              <span
                key={idx}
                className="px-3.5 py-1.5 rounded-full text-xs sm:text-sm font-semibold bg-accent-blue/10 text-accent-blue border border-accent-blue/20 hover:bg-accent-blue/20 transition-colors shadow-xs"
              >
                {tech}
              </span>
            )) || (
