...
function InstitutionLogo({
  ...
  if (iconName) {
    const IconComponent = (FaIcons as Record<string, React.ComponentType<{ className?: string }>>)[iconName] || FaGraduationCap;
    return <IconComponent className="h-10 w-10" />;
  }

  if (!logo || !logo.light) return <FaGraduationCap className="h-10 w-10" />;
